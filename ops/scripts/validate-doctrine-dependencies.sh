...
echo -e "${BLUE}[1/6] Checking for unparameterized 'work/' references...${NC}"
...
echo -e "${BLUE}[2/6] Checking for unparameterized 'docs/' references...${NC}"
...
echo -e "${BLUE}[3/6] Checking for unparameterized 'specifications/' references...${NC}"
...
echo -e "${BLUE}[4/6] Checking for tool-specific directory references...${NC}"
...
echo -e "${BLUE}[5/6] Checking for invalid path variables...${NC}"
VALID_VARS=('WORKSPACE_ROOT' 'DOC_ROOT' 'SPEC_ROOT' 'OUTPUT_ROOT')
INVALID_VARS=$(grep -rn '\${[A-Z_]*}' "$DOCTRINE_DIR" --include="*.md" --include="*.yaml" --include="*.yml" \
    | grep -v '\${WORKSPACE_ROOT}\|\${DOC_ROOT}\|\${SPEC_ROOT}\|\${OUTPUT_ROOT}' \
...
# Check 6: Template examples with .github/agents (warnings only)
echo -e "${BLUE}[6/6] Checking template examples...${NC}"
