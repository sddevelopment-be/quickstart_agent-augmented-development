...
class MetricsExtractor:
    ...
    def _parse_work_log(self, log_file: Path) -> Optional[Dict[str, Any]]:
        ...
        if metadata_section:
            # Duration
            duration_match = re.search(r'\*\*Duration:\*\*\s*(\d+)\s*minutes?', metadata_section)
            if duration_match:
                metrics['duration_minutes'] = int(duration_match.group(1))
            
            # Token usage
            token_match = re.search(r'\*\*Token Usage:\*\*\s*([0-9,]+)\s*input\s*\+\s*([0-9,]+)\s*output\s*=\s*([0-9,]+)\s*total', metadata_section)
            if token_match:
            ...
            context_match = re.search(r'\*\*Context Files Loaded:\*\*\s*(\d+)', metadata_section)
            if context_match:
                metrics['context_files_loaded'] = int(context_match.group(1))
            
            # Artifacts created/modified
            artifacts_created_match = re.search(r'\*\*Artifacts Created:\*\*\s*(\d+)', metadata_section)
            if artifacts_created_match:
                metrics['artifacts_created'] = int(artifacts_created_match.group(1))
            
            artifacts_modified_match = re.search(r'\*\*Artifacts Modified:\*\*\s*(\d+)', metadata_section)
    ...
    def output_json(self, output_file: Optional[Path] = None) -> None:
    ...
    def output_csv(self, output_file: Optional[Path] = None) -> None:
    ...
    def _calculate_summary(self) -> Dict[str, Any]:
        ...
        for metric in self.metrics:
            ...
            if agent not in summary['agents']:
                summary['agents'][agent] = {'count': 0, 'total_duration': 0, 'total_tokens': 0}
...
def main():
    ...
    if not metrics:
        ...
        if args.verbose:
            print("Check that work logs and task files contain ADR-009 metrics", file=sys.stderr)
