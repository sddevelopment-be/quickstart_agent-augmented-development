...
class AgentBase(ABC):
    ...
    def create_result_block(
        ...
        next_task_notes: Optional[List[str]] = None,
    ) -> Dict[str, Any]:
        """
    ...
    def create_work_log(
        ...
        outcomes: str,
        lessons_learned: str,
        directives_used: Optional[Dict[str, Any]] = None,
        ...
        slug = self.current_task["id"].split("-", 3)[-1] if "id" in self.current_task else "work"
        log_filename = f"{timestamp.strftime('%Y-%m-%dT%H%M')}-{self.agent_name}-{slug}.md"
