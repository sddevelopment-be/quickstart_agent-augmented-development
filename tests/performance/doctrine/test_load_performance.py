...
class TestLoadPerformance:
    ...
    def test_load_agents_performance(self, agent_files: list[Path]):
        ...
        print(
            f"   Throughput: {len(agents) / (elapsed_ms / 1000) if elapsed_ms > 0 else 0:.1f} agents/sec"
        )
        ...
        if len(agents) >= 20:
            ...
            assert (
                elapsed_ms < target_ms * 2
            ), f"Loading too slow: {elapsed_ms:.2f}ms > {target_ms * 2}ms"
    ...
    def test_load_directives_performance(self, directive_files: list[Path]):
        ...
        print(f"   Total time: {elapsed_ms:.2f}ms")
        print(
            f"   Time per directive: {elapsed_ms / len(directives) if directives else 0:.2f}ms"
        )
        print(
            f"   Throughput: {len(directives) / (elapsed_ms / 1000) if elapsed_ms > 0 else 0:.1f} directives/sec"
        )
    ...
    def test_validation_performance(
        ...
        assert (
            elapsed_ms < target_ms * 5
        ), f"Validation too slow: {elapsed_ms:.2f}ms > {target_ms * 5}ms"
...
class TestMemoryUsage:
    ...
    def test_memory_usage_estimate(self, agent_files: list[Path]):
        ...
        assert (
            avg_size_kb < 100
        ), f"Agent memory usage too high: {avg_size_kb:.2f} KB per agent"
