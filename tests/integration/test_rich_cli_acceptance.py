...
class TestRichCLIAcceptance:
    ...
    def test_config_validate_shows_error_in_rich_panel(self, tmp_path):
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ['--config-dir', str(config_dir), 'config', 'validate'])
    ...
    def test_no_color_flag_disables_rich_formatting(self, valid_config):
        ...
        assert '\x1b[' not in output, "Output contains ANSI color codes despite --no-color flag"
        assert '\033[' not in output, "Output contains ANSI color codes despite --no-color flag"
