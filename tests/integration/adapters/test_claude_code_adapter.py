...
class TestClaudeCodeAdapterIntegration:
    ...
    def fake_cli_config(self):
        ...
        return {"binary_path": str(FAKE_CLI_PATH), "timeout": 30}
    ...
    def test_successful_execution_plain_text(self, fake_cli_config):
        ...
        response = adapter.execute(
            prompt="Write a function to add two numbers", model="claude-3-opus"
        )
    ...
    def test_successful_execution_with_different_models(self, fake_cli_config):
        ...
        for model in models:
            response = adapter.execute(prompt="Test prompt", model=model)
    ...
    def test_successful_execution_json_output(self, fake_cli_config):
        ...
        fake_cli_config["template"] = (
            '{{binary}} --model {{model}} --prompt "{{prompt}}" --format json'
        )

        adapter = ClaudeCodeAdapter(fake_cli_config)

        response = adapter.execute(prompt="Write a pytest test", model="claude-3-opus")
    ...
    def test_invalid_model_error(self, fake_cli_config):
        ...
        with pytest.raises(InvalidModelError) as exc_info:
            adapter.execute(prompt="Test", model="gpt-4")  # Unsupported model
    ...
    def test_cli_error_handling_api_error(self, fake_cli_config):
        ...
        fake_cli_config["template"] = (
            '{{binary}} --model {{model}} --prompt "{{prompt}}" --error api-error'
        )
        ...
        assert (
            "api" in response.stderr.lower()
            or "authentication" in response.stderr.lower()
        )
    ...
    def test_timeout_handling(self, fake_cli_config):
        ...
        fake_cli_config["template"] = (
            '{{binary}} --model {{model}} --prompt "{{prompt}}" --delay 2'
        )
        ...
        assert response.status == "error"
        assert (
            "timeout" in response.stderr.lower()
            or "timed out" in response.stderr.lower()
        )
    ...
    def test_binary_not_found_error(self):
        ...
        config = {"binary_path": "/nonexistent/path/claude-code"}
    ...
    def test_output_parsing_json_format(self, fake_cli_config):
        ...
        response = adapter.execute(prompt="Write a test", model="claude-3-opus")
    ...
    def test_execution_with_special_characters_in_prompt(self, fake_cli_config):
        ...
        response = adapter.execute(prompt=special_prompt, model="claude-3-opus")
...
class TestClaudeCodeAdapterBinaryResolutionIntegration:
    ...
    def test_explicit_binary_path_resolution(self):
        ...
        config = {"binary_path": str(FAKE_CLI_PATH)}
    ...
    def test_tilde_expansion_in_binary_path(self, tmp_path):
        ...
        try:
            config = {"binary_path": "~/.local/bin/fake_claude_test"}
...
class TestClaudeCodeAdapterErrorScenarios:
    ...
    def test_cli_returns_non_zero_exit_code(self, fake_cli_config):
        ...
        fake_cli_config["template"] = (
            '{{binary}} --model {{model}} --prompt "{{prompt}}" --error invalid-model'
        )
    ...
    def test_empty_output_handling(self, fake_cli_config):
        ...
        response = adapter.execute(prompt="", model="claude-3-opus")
