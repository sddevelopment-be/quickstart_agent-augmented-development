...
@pytest.fixture
def sample_tasks(
    temp_work_dir: Path, sample_spec_with_frontmatter: Path
) -> dict[str, Path]:
...
class TestTaskLinking:
    ...
    def test_link_tasks_to_features(
        self,
        temp_work_dir: Path,
        sample_spec_with_frontmatter: Path,
        sample_tasks: dict[str, Path],
    ):
        """
...
class TestPortfolioAPI:
    ...
    def test_portfolio_endpoint_handles_no_specifications(
        self, temp_work_dir: Path, tmp_path: Path
    ):
        """
...
class TestPerformance:
    ...
    def test_uncached_portfolio_load_performance(
        ...
        with app.test_client() as client:
            ...
            assert (
                elapsed_ms < 500
            ), f"Uncached load took {elapsed_ms:.2f}ms (target: <500ms)"
    ...
    def test_cached_portfolio_load_performance(
        ...
        with app.test_client() as client:
            ...
            assert (
                elapsed_ms < 50
            ), f"Cached load took {elapsed_ms:.2f}ms (target: <50ms)"
