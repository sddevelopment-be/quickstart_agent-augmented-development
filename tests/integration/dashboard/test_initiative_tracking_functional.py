...
"""
        ...
        # Create Flask app
        app, _ = create_app(
            {"SPEC_DIR": str(spec_dir), "WORK_DIR": str(work_dir), "TESTING": True}
        )
        ...
        with app.test_client() as client:
            ...
            assert (
                initiative["id"] == "test-initiative"
            )  # Slugified from "Test Initiative"
...
"""
        ...
        feat1_progress = calculator.calculate_feature_progress(
            feature_tasks["feat-001"]
        )
        assert feat1_progress == 100  # Both tasks done

        feat2_progress = calculator.calculate_feature_progress(
            feature_tasks["feat-002"]
        )
