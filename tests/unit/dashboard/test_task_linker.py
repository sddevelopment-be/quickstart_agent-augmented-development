...
class TestTaskScanning:
    ...
    def test_scan_work_directory_structure(self, tmp_path: Path):
        ...
        (work_dir / "assigned" / "agent1" / "task-002.yaml").write_text(yaml.dump(task2))
...
class TestTaskGrouping:
    ...
    def test_group_tasks_by_specification(self, tmp_path: Path):
        ...
        task1 = {"id": "task-001", "specification": "specs/spec-a.md", "status": "inbox"}
        (work_dir / "inbox" / "task-001.yaml").write_text(yaml.dump(task1))
        
        task2 = {"id": "task-002", "specification": "specs/spec-a.md", "status": "inbox"}
    ...
    def test_group_tasks_by_feature(self, tmp_path: Path):
        ...
        task1 = {"id": "task-001", "specification": "specs/spec.md", "feature": "feat-001", "status": "inbox"}
        (work_dir / "inbox" / "task-001.yaml").write_text(yaml.dump(task1))
        
        task2 = {"id": "task-002", "specification": "specs/spec.md", "feature": "feat-001", "status": "inbox"}
        (work_dir / "inbox" / "task-002.yaml").write_text(yaml.dump(task2))
        
        task3 = {"id": "task-003", "specification": "specs/spec.md", "feature": "feat-002", "status": "inbox"}
        (work_dir / "inbox" / "task-003.yaml").write_text(yaml.dump(task3))
...
class TestOrphanTasks:
    ...
    def test_identify_orphan_tasks(self, tmp_path: Path):
        ...
        # Linked task
        linked = {"id": "linked-001", "specification": "specs/spec.md", "status": "inbox"}
...
class TestLinkingIntegration:
    ...
    def test_link_tasks_to_specification_metadata(self, tmp_path: Path):
        ...
        from src.llm_service.dashboard.task_linker import TaskLinker
        from src.llm_service.dashboard.spec_parser import SpecificationParser, SpecificationMetadata
...
"""
        ...
        # Create tasks
        task1 = {"id": "task-001", "specification": "test.md", "feature": "feat-001", "status": "done"}
        (work_dir / "inbox" / "task-001.yaml").write_text(yaml.dump(task1))
        
        task2 = {"id": "task-002", "specification": "test.md", "feature": "feat-001", "status": "in_progress"}
        (work_dir / "inbox" / "task-002.yaml").write_text(yaml.dump(task2))
...
class TestErrorHandling:
    ...
    def test_handle_malformed_task_yaml(self, tmp_path: Path):
        ...
        # Invalid YAML
        (work_dir / "inbox" / "bad.yaml").write_text("id: task-001\ntitle: Missing quote")
