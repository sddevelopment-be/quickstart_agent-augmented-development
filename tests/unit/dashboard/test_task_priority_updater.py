...
"""
    ...
    def test_update_priority_preserves_field_order(self, sample_task_yaml):
        ...
        id_line = next(
            i for i, line in enumerate(original_lines) if line.startswith("id:")
        )
        status_line = next(
            i for i, line in enumerate(original_lines) if line.startswith("status:")
        )
        priority_line = next(
            i for i, line in enumerate(original_lines) if line.startswith("priority:")
        )
        ...
        new_id_line = next(
            i for i, line in enumerate(updated_lines) if line.startswith("id:")
        )
        new_status_line = next(
            i for i, line in enumerate(updated_lines) if line.startswith("status:")
        )
        new_priority_line = next(
            i for i, line in enumerate(updated_lines) if line.startswith("priority:")
        )
    ...
    def test_optimistic_locking_with_timestamp(self, sample_task_yaml):
        ...
        with pytest.raises(ConcurrentModificationError):
            updater.update_task_priority(
                sample_task_yaml.name, "CRITICAL", last_modified=stale_timestamp
            )
    ...
    def test_optimistic_locking_allows_current_timestamp(self, sample_task_yaml):
        ...
        updater.update_task_priority(
            sample_task_yaml.name, "HIGH", last_modified=current_time
        )
    ...
    def test_update_adds_last_modified_timestamp(self, sample_task_yaml):
        ...
        timestamp_match = re.search(
            r'last_modified: ["\']?(\d{4}-\d{2}-\d{2}T[\d:]+.*Z)["\']?', updated_content
        )
    ...
    def test_multiline_description_preserved(self, sample_task_yaml):
        ...
        assert (
            "description: |" in updated_content or "description: >" in updated_content
        )
