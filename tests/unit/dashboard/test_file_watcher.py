...
class TestFileWatcher:
    ...
    def test_get_current_task_snapshot(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Create sample tasks
            (inbox_dir / 'task1.yaml').write_text(yaml.dump({'id': 'task1', 'status': 'new'}))
            (assigned_dir / 'task2.yaml').write_text(yaml.dump({'id': 'task2', 'status': 'assigned'}))
            (done_dir / 'task3.yaml').write_text(yaml.dump({'id': 'task3', 'status': 'done'}))
