...
def test_task_yaml_can_be_parsed():
    ...
    """
    task_file = Path(
        "work/collaboration/done/frontend-freddy/2026-02-10T1105-frontend-freddy-kanban-restructure.yaml"
    )
    ...
    try:
        ...
        print(
            f"✅ PASS: Task YAML parsed successfully ({len(data['acceptance_criteria'])} criteria)"
        )
        return True
        ...
        print(f"   Error: {e}")
        print("   Fix: Properly quote or escape strings in list items")
        return False
    ...
    for spec_path in spec_files:
        ...
        try:
            ...
            if "initiative" not in frontmatter:
                ...
                print(
                    "   Fix: Add 'initiative: framework-distribution' to frontmatter"
                )
                results.append(False)
            else:
                print(
                    f"✅ PASS: {spec_file.name} - Has initiative field: {frontmatter['initiative']}"
                )
                results.append(True)
