...
class TestRoutingEngineExecute:
    ...
    def test_routing_engine_execute_uses_correct_adapter(self, test_config):
        ...
        with patch("shutil.which", return_value="/usr/bin/mock"):
            ...
            with patch.object(
                engine.adapters["claude-code"].subprocess_wrapper,
                "execute",
                return_value=mock_result,
            ):
                response = engine.execute(
                    agent_name="test-agent", prompt="Write code", model="claude-3-opus"
                )
    ...
    def test_routing_engine_execute_with_routing_decision(self, test_config):
        ...
        with patch("shutil.which", return_value="/usr/bin/mock"):
            ...
            with patch.object(
                ...
                response = engine.execute(agent_name="test-agent", prompt="test prompt")
...
class TestAddToolViaYAML:
    ...
    def test_add_new_tool_gemini_via_yaml(self):
        ...
        with patch("shutil.which", return_value="/usr/bin/google-ai"):
            engine = RoutingEngine(
                config["agents"], config["tools"], config["models"], config["policies"]
            )
            ...
            with patch.object(
                gemini_adapter.subprocess_wrapper, "execute", return_value=mock_result
            ):
