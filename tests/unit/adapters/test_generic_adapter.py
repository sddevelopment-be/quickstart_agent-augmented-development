...
class TestGenericYAMLAdapterValidateConfig:
    ...
    def test_validate_config_fails_without_binary(self):
        ...
        config = {"command_template": "{{binary}} {{prompt}}", "models": ["model-1"]}

        with patch("shutil.which", return_value="/usr/bin/tool"):
            adapter = GenericYAMLAdapter(
                "tool", {"binary": "tool", "command_template": "test", "models": []}
            )
    ...
    def test_validate_config_fails_without_command_template(self):
        ...
        config = {"binary": "tool", "models": ["model-1"]}
    ...
    def test_validate_config_fails_without_models(self):
        ...
        config = {"binary": "tool", "command_template": "{{binary}} {{prompt}}"}
...
class TestGenericYAMLAdapterBinaryResolution:
    ...
    def test_binary_resolution_raises_error_when_not_found(self):
        ...
        from src.llm_service.adapters.generic_adapter import (
            BinaryNotFoundError,
            GenericYAMLAdapter,
        )
...
class TestGenericYAMLAdapterExecute:
    ...
    def test_execute_success_with_valid_model(self):
        ...
        with patch.object(
            adapter.subprocess_wrapper, "execute", return_value=mock_result
        ):
    ...
    def test_execute_uses_template_parser(self):
        ...
        with (
            patch.object(adapter.template_parser, "parse") as mock_parse,
            patch.object(
                adapter.subprocess_wrapper, "execute", return_value=mock_result
            ),
        ):
    ...
    def test_execute_uses_output_normalizer(self):
        ...
        with (
            patch.object(
                adapter.output_normalizer, "normalize", return_value=mock_normalized
            ) as mock_norm,
            patch.object(
                adapter.subprocess_wrapper, "execute", return_value=mock_result
            ),
