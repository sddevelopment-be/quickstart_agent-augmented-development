...
from src.domain.collaboration.types import TaskMode, TaskPriority, TaskStatus
from src.domain.doctrine.types import get_all_agents, validate_agent
...
class TestTaskStatusStateMachine:
    ...
    def test_self_transition_not_allowed(self):
        ...
        for status in TaskStatus:
            ...
            ), f"{status} should not transition to itself"
    ...
    def test_all_non_terminal_states_have_error_transition(self):
        ...
        for status in non_terminal:
            assert (
                TaskStatus.ERROR in status.valid_transitions()
    ...
    def test_terminal_states_have_no_transitions(self):
        ...
        for status in terminal:
            assert (
                len(status.valid_transitions()) == 0
