...
|-------------------------------------------|----------------------------|---------------------------------------|------------|
| `.github/agents/architect.agent.md`       | Local Customization        | Manual merge; relocate custom to `local/` | Medium     |
| `.github/agents/directives/010_*.md`      | Non-Breaking Addition      | Safe to adopt; append new sections    | Low        |
| `docs/templates/architecture/ADR.md`      | Breaking Change            | ❗️ Requires manual review            | High       |
...
1. **Breaking Changes in Core Governance:**
   - **File:** `.github/agents/directives/006_version_governance.md`
...
2. **Template Structure Change:**
   - **File:** `docs/templates/architecture/ADR.md`
...
```diff
--- .github/agents/architect.agent.md (current)
+++ .github/agents/architect.agent.md.framework-new
...
### 3. `.github/agents/directives/010_mode_protocol.md`
...
```diff
--- .github/agents/directives/010_mode_protocol.md (current)
+++ .github/agents/directives/010_mode_protocol.md.framework-new
@@ Line 85
+## Primer Integration
+
...
### 4. `docs/templates/architecture/ADR.md`
...
```diff
--- docs/templates/architecture/ADR.md (current)
+++ docs/templates/architecture/ADR.md.framework-new
@@ Line 15
-## Decision
+## Decision Rationale
+
...
|---------------------------------------------|---------------------------------------------|-------------------------------------------------|
| `.github/agents/architect.agent.md`         | `local/agents/architect-custom.agent.md`    | Custom directive descriptions                   |
| `.github/agents/directives/020_*.md`        | `local/directives/020_custom_leniency.md`   | Project-specific leniency rules                 |
| `docs/templates/prompts/task.md`            | `local/templates/prompts/task-extended.md`  | Custom task metadata fields                     |

### Relocation Procedure

1. **Copy custom version to `local/` path:**
   ```bash
   cp .github/agents/architect.agent.md local/agents/architect-custom.agent.md
   ```

2. **Adopt framework version in original location:**
   ```bash
   mv .github/agents/architect.agent.md.framework-new .github/agents/architect.agent.md
   ```
...
| Path                                      | Type               | Notes                                  |
|-------------------------------------------|--------------------|----------------------------------------|
| `.github/agents/framework-guardian.agent.md` | New Agent Profile  | Introduced in framework version `X.Y.Z` |
| `.github/agents/directives/025_*.md`      | New Directive      | Framework Guardian workflow            |
| `docs/templates/GUARDIAN_AUDIT_REPORT.md` | New Template       | Audit report format                    |
...
- `docs/templates/prompts/primer.md`
...
2. **Consult distribution docs:** `docs/architecture/design/`
