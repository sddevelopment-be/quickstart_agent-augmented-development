...
def test_extract_metrics_from_work_logs(
    temp_work_env: Path, sample_work_log: Path
) -> None:
    """
    ...
    assert (
        "Metadata" in log_content
    ), "Test precondition: log should have Metadata section"
...
def test_extract_metrics_from_task_files(
    temp_work_env: Path, sample_task_yaml: Path
) -> None:
    """
    ...
    assert "done" in str(
        sample_task_yaml
    ), "Test precondition: task should be in done directory"
...
def test_json_output_format(
    temp_work_env: Path, sample_work_log: Path, tmp_path: Path
) -> None:
    """
...
def test_empty_work_directory(tmp_path: Path) -> None:
    ...
    assert not (
        empty_dir / "reports" / "logs"
    ).exists(), "Test precondition: no logs directory"
...
def test_work_log_without_metrics(temp_work_env: Path) -> None:
    ...
    assert (
        "Metrics" not in log_file.read_text()
    ), "Test precondition: should have no metrics section"
