...
3. Constitution / `.doctrine-config/` (project-specific customization — may extend, not contradict)
4. Doctrine Directives (explicit constraints — apply unless Constitution narrows scope)
5. Mission guidance (workflow context) — domain-specific behavior
6. Doctrine Tactics/Templates (execution patterns) — how-to guidance

⚠️ **Key constraint:** Constitution sits at the local-override level per the Doctrine instruction hierarchy (AGENTS.md). It customizes within Doctrine bounds, not above them.
...
**Decision:** Doctrine core guidelines are immutable; Constitution customizes within their bounds.

The Doctrine framework's instruction hierarchy (AGENTS.md) is explicit and immutable:
...
4. **Local Doctrine Overrides** (MEDIUM) — `.doctrine-config/` (and by extension, Constitution)

Constitution is the **Spec Kitty equivalent of `.doctrine-config/`** (see Q3a). It sits at the local-override level — it can customize, extend, and add project-specific rules, but it **MUST NOT contradict** General or Operational Guidelines. This is consistent with both frameworks: Doctrine's bootstrap.md states local overrides "MUST NOT override general_guidelines.md or operational_guidelines.md"; Spec Kitty's Constitution is project-scoped, not framework-scoped.

**Implementation:**
- Constitution template includes a precedence declaration that acknowledges Doctrine authority:
  ```markdown
  ...
  1. Doctrine General Guidelines (immutable framework values)
  2. Doctrine Operational Guidelines (immutable behavioral norms)
  3. This Constitution / .doctrine-config (project-specific rules)
  4. Doctrine Directives (explicit constraints — apply unless this Constitution narrows scope)
  5. Mission-specific workflow guidance
  6. Doctrine Tactics and Templates (execution patterns)
  
  This Constitution customizes Doctrine for this project.
  It may narrow, extend, or add to Doctrine rules but cannot contradict
  General or Operational Guidelines.
  ```

- `DoctrineGovernancePlugin.validate_*()` loads General/Operational Guidelines first, then applies Constitution/`.doctrine-config/` overrides, then evaluates directives
- Conflict detection: if Constitution contradicts a General/Operational Guideline, the plugin emits a `warn` with remediation guidance
...
1. **Doctrine General Guidelines** — Immutable framework values
2. **Doctrine Operational Guidelines** — Immutable behavioral norms
3. **This Constitution** — Project-specific rules and customization
4. **Doctrine Directives** — Explicit constraints (apply unless this Constitution narrows scope)
5. **Mission-specific guidance** — Workflow context from active mission
6. **Doctrine Tactics and Templates** — Execution patterns and output contracts

**This Constitution customizes Doctrine for this project.**
It may narrow, extend, or add to Doctrine rules but cannot contradict
General or Operational Guidelines.

**When in doubt, consult General Guidelines first, then this Constitution, then relevant directives.**
