...
class AgentConfig(BaseModel):
    ...
    task_types: dict[str, str] = Field(
        default_factory=dict, description="Task type to model mapping"
    )
...
class ToolConfig(BaseModel):
    ...
    platforms: PlatformPaths | None = Field(
        default=None, description="Platform-specific binary paths"
    )
    ...
    env_required: list[str] | None = Field(
        default=None,
...
class ModelConfig(BaseModel):
    ...
    task_suitability: list[str] = Field(
        default_factory=list, description="Suitable task types"
    )
...
class BudgetLimit(BaseModel):
    ...
    type: Literal["soft", "hard"] = Field(
        "soft", description="Enforcement type: 'soft' warns, 'hard' blocks"
    )
    threshold_percent: int = Field(
        80, ge=0, le=100, description="Warn/block threshold percentage"
    )
...
class CostOptimization(BaseModel):
    ...
    simple_task_threshold_tokens: int = Field(
        1500, gt=0, description="Token threshold for simple task classification"
    )
    simple_task_models: list[str] = Field(
        default_factory=list, description="Models to use for simple tasks"
    )
    complex_task_models: list[str] = Field(
        default_factory=list, description="Models to use for complex tasks"
    )
...
class RateLimiting(BaseModel):
    ...
    max_requests_per_minute: dict[str, int] | None = Field(
        default_factory=dict, description="Per-tool rate limits (requests/minute)"
    )
    max_requests_per_hour: dict[str, int] | None = Field(
        default_factory=dict, description="Per-tool rate limits (requests/hour)"
    )
...
class PolicyConfig(BaseModel):
    ...
    monthly_budget_usd: float | None = Field(
        None, ge=0, description="Monthly budget limit (USD)"
    )
    limit: BudgetLimit = Field(
        default_factory=BudgetLimit, description="Budget enforcement"
    )
    prefer_cheaper_models_under_tokens: int | None = Field(
        None,
        gt=0,
        description="Prefer cheaper models for prompts under this token count",
    )
    auto_fallback_on_rate_limit: bool = Field(
        True, description="Auto-fallback on rate limits"
    )
    ...
    require_justification_over_usd: float | None = Field(
        None, ge=0, description="Require justification for requests over this cost"
    )
...
class PoliciesSchema(BaseModel):
    ...
    cost_optimization: CostOptimization | None = None
...
class TelemetryConfig(BaseModel):
    ...
    enabled: bool = Field(True, description="Enable/disable telemetry logging")
    db_path: str = Field(
        "~/.llm-service/telemetry.db", description="Path to SQLite telemetry database"
    )
    ...
    retention_days: int = Field(
        30, ge=0, description="Days to retain detailed logs (0 = keep forever)"
    )
...
def validate_agent_references(
    agents: AgentsSchema, tools: ToolsSchema, models: ModelsSchema
) -> list[str]:
    """
    ...
    for agent_name, agent_config in agents.agents.items():
        ...
        if (
            agent_config.preferred_tool in tools.tools
            and agent_config.preferred_model in models.models
        ):
