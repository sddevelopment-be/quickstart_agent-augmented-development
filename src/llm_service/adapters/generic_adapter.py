...
from .subprocess_wrapper import SubprocessWrapper, CommandNotFoundError
from .output_normalizer import OutputNormalizer
from ..config.env_utils import expand_env_vars, validate_required_env_vars, EnvVarNotFoundError
...
class GenericYAMLAdapter(ToolAdapter):
    ...
    def execute(self, prompt: str, model: str, **kwargs) -> ToolResponse:
        ...
        if result.exit_code != 0:
            return ToolResponse(
                ...
                exit_code=result.exit_code,
                stdout=result.stdout,
                stderr=result.stderr or f"Command failed with exit code {result.exit_code}",
                duration_seconds=result.duration_seconds,
            )
    ...
    def _resolve_binary_path(self) -> str:
        ...
        if "platforms" in self.tool_config and self.tool_config["platforms"]:
            ...
            if config_key:
                ...
                paths_to_try = []
                if isinstance(platforms, dict) and config_key in platforms and platforms[config_key]:
                    paths_to_try.append(platforms[config_key])
                if system == "Darwin" and isinstance(platforms, dict) and "darwin" in platforms and platforms["darwin"]:
    ...
    def _format_binary_not_found_error(self) -> str:
        ...
        message = [
            ...
            f"2. Specify explicit path in configuration:",
            "   tools:",
            f"     {self._tool_name}:",
