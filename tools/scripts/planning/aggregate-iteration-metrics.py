...
class IterationMetrics:
    ...
    errors: int
    framework_health_score: float | None = None
    architectural_alignment: float | None = None
...
class IterationMetricsAnalyzer:
    ...
    def _parse_iteration_summary(self, filepath: Path) -> IterationMetrics | None:
        ...
        try:
            ...
            tasks_completed = self._extract_metric(
                content, r"\| Tasks Completed \| (\d+)/(\d+)"
            )
            agents_utilized = self._extract_metric(
                content, r"\| Agents Utilized \| (\d+)"
            )
            artifacts_created = self._extract_metric(
                content, r"\| Artifacts Created \| (\d+)"
            )
            work_logs = self._extract_metric(
                content, r"\| Work Logs Generated \| (\d+)"
            )
            validation = self._extract_metric(
                content, r"\| Validation Success \| (\d+)%"
            )
            ...
            health_score = self._extract_optional_metric(
                content, r"\| Framework Health Score \| (\d+)/100"
            )
            alignment = self._extract_optional_metric(
                content, r"\| Architectural Alignment \| ([\d.]+)%"
            )
    ...
    def _extract_metric(self, content: str, pattern: str) -> str | None:
    ...
    def _extract_optional_metric(self, content: str, pattern: str) -> float | None:
        """Extract optional metric, return None if not found."""
        match = re.search(pattern, content, re.IGNORECASE)
        return float(match.group(1)) if match else None

    def _parse_duration(self, duration_str: str | None) -> int:
    ...
    def _extract_agent_breakdown(self, content: str) -> dict[str, int]:
        ...
        if agent_section:
            ...
            for match in re.finditer(
                r"### ([^\n]+)\n- \*\*Tasks Completed\*\*: (\d+)", section_text
            ):
                ...
                if (
                    "observations" not in agent_name.lower()
                    and "performance" not in agent_name.lower()
                ):
    ...
    def aggregate_agent_metrics(self) -> None:
        ...
        for _date, token_data in self.token_data.items():
            per_agent = token_data.get("per_agent_metrics", {})
            for agent_name, metrics in per_agent.items():
                if agent_name in self.agent_metrics:
                    self.agent_metrics[agent_name].total_tokens += metrics.get(
                        "total_tokens", 0
                    )
    ...
    def _generate_markdown_report(self) -> str:
        ...
        if self.iterations:
            ...
            avg_duration = sum(it.duration_minutes for it in self.iterations) / len(
                self.iterations
            )
            ...
            lines.append(
                f"- **Average Iteration Duration:** {avg_duration:.1f} minutes"
            )
            lines.append(
                f"- **Total Errors:** {sum(it.errors for it in self.iterations)}"
            )
        lines.append("")
        lines.append("---")
        lines.append("")
        ...
        lines.append("")
        lines.append(
            "| Iter | Date | Duration | Tasks | Completion | Agents | Artifacts | Errors | Health |"
        )
        lines.append(
            "|------|------|----------|-------|------------|--------|-----------|--------|--------|"
        )
        ...
        for it in self.iterations:
            health = (
                f"{it.framework_health_score:.0f}/100"
                if it.framework_health_score
                else "N/A"
            )
            lines.append(
        ...
        for agent_name in sorted(self.agent_metrics.keys()):
            agent = self.agent_metrics[agent_name]
            avg_tokens = (
                agent.total_tokens / agent.total_tasks if agent.total_tasks > 0 else 0
            )
    ...
    def _calculate_trends(self) -> dict:
        ...
        if len(self.iterations) < 2:
            return {
                "insights": [
                    "Not enough data for trend analysis (need at least 2 iterations)"
                ]
            }
        ...
        if completion_rates[-1] > completion_rates[0]:
            insights.append(
                f"✅ Completion rate improving: {completion_rates[0]:.0f}% → {completion_rates[-1]:.0f}%"
            )
        elif completion_rates[-1] < completion_rates[0]:
            insights.append(
                f"⚠️ Completion rate declining: {completion_rates[0]:.0f}% → {completion_rates[-1]:.0f}%"
            )
        else:
        ...
        if durations[-1] < durations[0]:
            insights.append(
                f"✅ Task duration decreasing: {durations[0]:.1f}m → {durations[-1]:.1f}m per task"
            )
        elif durations[-1] > durations[0]:
            insights.append(
                f"⚠️ Task duration increasing: {durations[0]:.1f}m → {durations[-1]:.1f}m per task"
            )
        ...
        if self.agent_metrics:
            ...
            insights.append(
                f"🏆 Most active agent: {most_active.name} ({most_active.total_tasks} tasks)"
            )
...
Examples:
  ...
  ./aggregate-iteration-metrics.py --repo-root /path/to/repo
    ...
    if not args.repo_root.exists():
        print(
            f"❗️ Error: Repository root does not exist: {args.repo_root}",
            file=sys.stderr,
        )
        sys.exit(1)
