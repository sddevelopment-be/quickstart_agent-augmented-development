    ...
    try:
        ...
        if e.context:
            ...
            for ctx_error in e.context:
                ctx_path = (
                    " -> ".join(str(p) for p in ctx_error.path)
                    if ctx_error.path
                    else "root"
                )
            ...
            for agent_name, agent_config in config["agent"].items():
                ...
                if "prompt" in agent_config:
                    if not isinstance(agent_config["prompt"], str):
                        errors.append(
                            f"Agent '{agent_name}': 'prompt' must be a string"
                        )
...
class OpenCodeValidator:
    ...
    def get_report(self, is_valid: bool, error_msg: str = "") -> str:
        ...
        if is_valid:
            ...
            if self.schema:
                lines.append(
                    "Configuration is valid according to OpenCode specification."
                )
            else:
...
Exit codes:
  ...
  2 - File error (not found, invalid JSON, etc.)
    ...
    parser.add_argument(
        "config_file", type=Path, help="Path to OpenCode configuration JSON file"
    )
    ...
    parser.add_argument(
        "-q", "--quiet", action="store_true", help="Suppress output, only use exit code"
    )
    ...
    if not args.no_schema:
        ...
        if schema_path and not schema_path.exists():
            if not args.quiet:
                ...
                print("⚠️  Falling back to basic validation", file=sys.stderr)
            schema_path = None
